...
#Region "Utilities"
            ...
            Using Transaction As New TransactionScope(TransactionScopeOption.RequiresNew)
                ...
                Dim Req_IJ = From R In LMISDb.Issues
                                    Join IJ In LMISDb.InventoryJournals
                                        On R.InventoryJournalID Equals IJ.ID
                                Where (R.ID = CMBX_IssuedID.Text)
                                    Select R.Requisition.InventoryJournal
                Iss_IJ.Remark = TBX_Remark.Text
                Iss_IJ.VoucherDate = DTP_VoucherDate.Value
                Iss_IJ.InventoryJournalStatusID = SaveStatus
                Dim IJID As String = Iss_IJ.ID
                If LMISDb.SaveChanges() > 0 Then
                    ...
                    LMISDb.SaveChanges()
                End If
                If TBX_ChangeInvoiceNo.Text <> String.Empty And TBX_ChangeInvoiceNo.Text <> CMBX_IssuedID.Text Then
                    Dim Iss = (From R In LMISDb.Issues
                                    Where (R.ID = CMBX_IssuedID.Text)
                                        Select R).Single
                    Dim NewIssue As New Issue With {
                        .ID = TBX_ChangeInvoiceNo.Text,
                        .RequisitionID = Iss.RequisitionID,
                        .InventoryJournalID = Iss.InventoryJournalID}
                    LMISDb.Issues.DeleteObject(Iss)
                    LMISDb.SaveChanges()
    ...
    Function ValidateDataForm() As Boolean
        ...
        Else
            Dim LMISDb As New LMISEntities
            Dim RequestingFacility = (From Iss In LMISDb.Issues Join Req In LMISDb.Requisitions On Iss.RequisitionID Equals Req.ID Join L In LMISDb.Departments On Req.DepartmentID Equals L.ID Where (Iss.ID = CMBX_IssuedID.Text) Select L).Single
            ...
            End If
        End If
        If CMBX_IssuedID.SelectedItem IsNot Nothing And TBX_ChangeInvoiceNo.Text <> String.Empty Then
            If (From Iss In (New LMISEntities).Issues Where Iss.ID = TBX_ChangeInvoiceNo.Text Select Iss).Count > 0 Then
                ERP_Error.SetError(CMBX_IssuedID, "Please select appropriate 'Request ID' from the list")
                No_Error = False
            End If
        End If
...
#Region "Events"
    ...
    Private Sub BTN_Save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTN_Save.Click
        ...
        If ValidateDataForm() Then
            If MessageBox.Show("Are you sure you want to save this Invoice", "Are you sure?", System.Windows.Forms.MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                If SaveData(1) Then
                    FRM_GLBMain.TLSL_MainStatus.Text = "Invoice Saved"
    ...
    Private Sub BTN_Print_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTN_Print.Click
        If ValidateDataForm() Then
            If MessageBox.Show("Are you sure you want Save this Invoice?", "Are you sure?", System.Windows.Forms.MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                Me.Cursor = Cursors.WaitCursor
                If SaveData(1) Then
                    FRM_GLBMain.TLSL_MainStatus.Text = "Invoice Saved"
                    MessageBox.Show("Invoice Saved", "Data Saved", MessageBoxButtons.OK, MessageBoxIcon.Information)
        ...
        If ValidateDataForm() Then
            If MessageBox.Show("Are you sure you want to save this Invoice", "Are you sure?", System.Windows.Forms.MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                ...
                If SaveData(2) Then
                    ...
                    Dim ReportParameter As New Dictionary(Of String, String())
                    ReportParameter.Add("ID", {CMBX_IssuedID.Text})
                    FRM_Reporter.Show(ReportTypes.IJInvoice, ReportParameter)
                    ClearForm()
                    LoadForm()
                Else
                    FRM_GLBMain.TLSL_MainStatus.Text = "Invoice NOT Saved"
    ...
    Private Sub BTN_Void_Click_1(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTN_Void.Click
        If ValidateDataForm() Then
            Me.Cursor = Cursors.WaitCursor
            If MessageBox.Show("Are you sure you want to invalidate this Invoice", "Are you sure?", System.Windows.Forms.MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                Try
                    Dim LMISDb As New LMISEntities
                    Dim Req_IJ = From R In LMISDb.Issues
                            Join IJ In LMISDb.InventoryJournals
    ...
    Private Sub BTN_Reject_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTN_Reject.Click
        ...
        If Validate_Reject_Data() Then
            If MessageBox.Show("Are you sure you want to reject the requisition", "Are you sure?", System.Windows.Forms.MessageBoxButtons.YesNo) = System.Windows.Forms.DialogResult.Yes Then
                Me.Cursor = Cursors.WaitCursor
                If Save_Reject() Then
                    FRM_GLBMain.TLSL_MainStatus.Text = "Requisition has been Rejected"
                    MessageBox.Show("Requisition has been Rejected", "Data Saved", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Dim ReportParameter As New Dictionary(Of String, String())
                    ReportParameter.Add("IssueID", {CMBX_IssuedID.Text})
                    FRM_Reporter.Show(ReportTypes.IJInvoiceRejected, ReportParameter)
                    ClearForm()
                    LoadForm()
                Else
                    FRM_GLBMain.TLSL_MainStatus.Text = "Requisition has been NOT Rejected"
                    MessageBox.Show("   Requisition has been NOT Rejected    ", "Error in Saving", MessageBoxButtons.OK, MessageBoxIcon.Error)
                End If
                Me.Cursor = Cursors.Default
            End If
        End If
...
#End Region
    ...
    Private Sub TBX_ChangeInvoiceNo_TextChanged(sender As System.Object, e As System.EventArgs) Handles TBX_ChangeInvoiceNo.TextChanged
